...
[`deploy.sh`](deploy.sh) делает всё то же, что описано ниже вручную, но
идемпотентно — и годится как для первой установки, так и для обновления.
В отличие от инструкции он ничего не предполагает о вашем сервере: путь
установки ищет на диске, пользователя сервиса берёт по владельцу каталога и
подставляет обоих в юнит. Останавливается там, где нужно решение человека:
незаполненный `.env`, расхождение рабочего дерева с `origin/main`,
недействительный токен.

Попутно снимает webhook, если тот висит (см. «Диагностика» — это самая частая
причина «процесс живой, а бот молчит»), и в конце ждёт в логе строку старта,
а не просто `systemctl is-active`.
